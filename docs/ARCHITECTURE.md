...
当前已经实现 overlay relay、Windows/Linux TUN 客户端、客户端 WebSocket 连接池和服务端连接池感知转发；exit gateway 仍是后续阶段。
...
- 跨平台 `iperf3` TCP 双向可通。
- 修复 standby 空闲读超时后，压测期间未再观察到无故 primary 轮换或重连风暴。
...
- 每连接心跳和连接池管理（自动重连、standby 补建、计划轮换、QoS 观测、背压、连接状态指标）。
...
- 正常情况下只有一条 primary 连接承载 ICMP、常规 UDP 和大多数 TCP 流量。
- 其余连接作为 standby 热备，已经完成 hello 注册，可用于快速切换、超时轮换、UDP 突发，以及 primary 退化/高压时的新 TCP 流突发。
- 每条连接有独立的 read/write/heartbeat goroutine。
- primary 断开时，池会提升 standby 为新的 primary 并补建 standby。
- 计划轮换只在显式配置轮换间隔，或超时探测器已经识别出稳定的 CDN/nginx 强制断开周期后启用；直连且稳定的 WebSocket 不会默认周期性重连。
- TCP 包按五元组维护轻量 flow 绑定；已有 TCP flow 固定在同一条 WebSocket 上，避免跨连接乱序。
- 新 TCP flow 默认进入 primary 的有界队列；当 primary 已退化或队列高压时，新 SYN 可先尝试 standby 非阻塞突发。
- TCP 队列满时阻塞 TUN 读取路径，让 TUN/系统 TCP 反馈自然降速。
...
- 当前不会仅因为单次写延迟或单次队列满就切换 primary；持续 critical 写延迟才会触发主动轮换。

拥塞控制：

- 不再在热路径通过令牌桶主动丢 TCP 包。
- TCP 使用 flow 绑定、有界队列和 WebSocket 写阻塞形成背压。
...
- 计划内 draining/rotation 关闭不会作为断连样本参与学习，避免轮换机制把自己的主动关闭误判成远端超时。
...
服务端把原始 IP 包按流量类型写入 Client B 的连接池。TCP 默认进入目标 VIP 的 inferred primary，并用 flow table 保持同一 TCP flow 不跨 WebSocket；当 inferred primary 高压时，新 TCP SYN 可尝试 standby 非阻塞突发，队列满时等待以保留背压。UDP 可在 primary 满时尝试 standby，仍不可用则快速丢弃。Client B 收到后写入 TUN，由 Client B 的操作系统网络栈处理。
...
- Linux -> Windows TCP 方向在远端高 RTT 测试中吞吐偏低且重传较多。
- 需要继续核查 Windows TUN 实际 MTU、写入路径、分片行为和 TCP MSS。
