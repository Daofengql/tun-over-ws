...
- 客户端创建 TUN（wireguard-go + wintun）。
- 配置虚拟 IP（netsh，不修改系统默认路由）。
- TUN -> WebSocket 数据泵。
- WebSocket -> TUN 数据泵。
- 单机两客户端互 ping 验证通过（`ping -S 10.66.0.2 10.66.0.3`）。
- `wintun.dll` 需放在二进制同目录。

待做：

- Linux 客户端 TUN 适配（`setup_linux.go`）。
- 优雅退出时清理路由和 TUN 设备。
